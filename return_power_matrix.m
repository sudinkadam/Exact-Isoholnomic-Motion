function W = return_power_matrix(x,k,l)
...
T2=[1,  0,  0.5*l*sin(alpha0)+0.5*l*sin(alpha2);
    0,  1,  -0.5*l*cos(alpha0)-0.5*l*sin(alpha2);
    0,  0,  1,];

E2=[0,      -0.5*l*sin(alpha2);
    0,      0.5*l*cos(alpha2);
    0,      -1];
...
Ruu = E0'*R0*E0+E1'*R1*E1+E2'*R2*E2;
Rbu = T0'*R0*E0+T1'*R1*E1+T2'*R2*E2;
Rbb = T0'*R0*T0+T1'*R1*T1+T2'*R2*T2;


W = Ruu - Rbu'*inv(Rbb)*Rbu;